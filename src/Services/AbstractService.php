...
namespace ConsulConfigManager\Consul\ACL\Services;
 ...
 * @package ConsulConfigManager\Consul\ACL\Services
 */
abstract class AbstractService
{
    ...
    private function initializeClient(): void
    {
        ...
        $this->client = Http::baseUrl(sprintf(
            ...
            'X-Consul-Token'    =>  Arr::get($activeConnection, 'access_token') ?? config('consul.acl.access_token', ''),
        ])->timeout(config('consul.acl.timeout', 15));
    }
    ...
    {
        $connectionName = config('consul.acl.default', 'default');
        $connectionsList = config('consul.acl.connections', [
            'default'       =>  [
                'scheme'    =>  'http',
                'host'      =>  '127.0.0.1',
                'port'      =>  8500,
        ...
        $withAutoSelect = config('consul.acl.auto_select', false);
        $withRandomServer = config('consul.acl.use_random', false);
